...
const (
	...
	ENV_LTEDU_AI_BASE_URL        = "LTEDU_AI_BASE_URL"
	ENV_LTEDU_AI_API_KEY         = "LTEDU_AI_API_KEY"
	ENV_LTEDU_AI_MODEL           = "LTEDU_AI_MODEL"
...
func initConfig() {
	...
	AI_BASE_URL := os.Getenv(ENV_LTEDU_AI_BASE_URL)
	AI_API_KEY := os.Getenv(ENV_LTEDU_AI_API_KEY)
	AI_MODEL := os.Getenv(ENV_LTEDU_AI_MODEL)
	...
	if AI_BASE_URL == "" {
		AI_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1" // Default DashScope API base URL
	}
	if AI_API_KEY == "" {
		AI_API_KEY = "your-dashscope-api-key" // Placeholder for DashScope API key
	}
	if AI_MODEL == "" {
		AI_MODEL = "qwen-plus" // Default model
	}
