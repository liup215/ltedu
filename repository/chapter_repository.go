...
err := r.db.Where("parent_id = ?", parentId).Find(&chapters).Error
return chapters, err
}

func (r *chapterRepository) FindBySyllabusID(syllabusId uint) ([]*model.Chapter, error) {
var chapters []*model.Chapter
err := r.db.Where("syllabus_id = ?", syllabusId).Find(&chapters).Error
return chapters, err
}
...
q.Count(&total)
err := q.Order("id DESC").Offset(offset).Limit(limit).Find(&chapters).Error
...
err := q.Order("id DESC").Find(&chapters).Error
return chapters, err
}
