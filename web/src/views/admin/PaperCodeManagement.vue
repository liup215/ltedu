<template>
  <div class="p-6">
    <header class="mb-6">
      <h1 class="text-3xl font-bold text-gray-900">{{ $t('paperCode.title') }}</h1>
      <p class="mt-1 text-sm text-gray-500">{{ $t('paperCode.subtitle') || '' }}</p>
    </header>
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 sm:space-x-4">
      <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
        ...
        <input 
          type="text" 
          v-model="searchQuery"
          :placeholder="$t('paperCode.searchByName')"
          class="px-3 py-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-base w-full sm:w-auto"
        />
      </div>
      <button 
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $t('paperCode.name') }}</th>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[180px]">{{ $t('paperCode.actions') }}</th>
          </tr>
        </thead>
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-if="loading">
            <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{{ $t('paperCode.loading') }}</td>
          </tr>
          <tr v-else-if="!paperCode.length">
            <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{{ $t('paperCode.noData') }}</td>
          </tr>
          <tr v-for="series in paperCode" :key="series.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
              <router-link
                :to="`/admin/paper-code/${series.id}/edit`"
                class="text-indigo-600 hover:text-indigo-900"
              >
                {{ $t('paperCode.edit') }}
              </router-link>
              <button 
                @click="deletePaperCode(series.id!)"
    ...
    <div v-if="!loading && totalItems > 0" class="mt-6 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
      <p class="text-sm text-gray-700">
        {{ $t('paperCode.pageInfo', { from: (currentPage - 1) * pageSize + 1, to: Math.min(currentPage * pageSize, totalItems), total: totalItems }) }}
      </p>
...
import paperService from '../../services/paperCodeService'
import syllabusService from '../../services/syllabusService'
import organisationService from '../../services/organisationService'
import qualificationService from '../../services/qualificationService'
import type { PaperCode, PaperCodeQuery } from '../../models/paperCode.model'
...
const fetchPaperCodeList = async () => {
  loading.value = true
  try {
    const query: PaperCodeQuery = {
    ...
    const response = await paperService.getPaperCodeList(query)
