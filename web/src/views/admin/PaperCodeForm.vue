  ...
  <div class="p-6">
    <header class="mb-6">
      <h1 class="text-3xl font-bold text-gray-900">
        {{ isEditMode ? $t('paperCode.edit') : $t('paperCode.add') }}
      </h1>
    </header>
      ...
      <div class="mb-6">
        ...
        >
          <option :value="0" disabled>
            {{ availableSyllabuses.length ? $t('paperCode.selectSyllabus') : $t('paperCode.noSyllabusMatch') }}
          </option>
...
const paperCodeId = computed(() => Number(route.params.id) || null)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditMode.value && paperCodeId.value) {
      await paperService.updatePaperCode({ ...payload, id: paperCodeId.value })
    } else {
      await paperService.createPaperCode(payload)
    }
...
onMounted(async () => {
  ...
  if (isEditMode.value && paperCodeId.value) {
    await fetchPaperCodeDetails(paperCodeId.value)
  }
})
</script>
