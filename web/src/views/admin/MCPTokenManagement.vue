<template>
  <div class="p-6">
    <header class="mb-6">
      <h1 class="text-3xl font-bold text-gray-900">MCP Token Management (Admin)</h1>
      <p class="mt-1 text-sm text-gray-500">
        Manage all MCP tokens across all users in the system.
      </p>
    </header>
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 sm:space-x-4">
      <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
        ...
        <button 
          type="button" 
          @click="applyFilters"
          class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 w-full sm:w-auto"
        >
          Apply Filters
        </button>
        <button 
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[180px]">
              Actions
            </th>
          </tr>
        </thead>
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div class="mt-2">
                  <p class="text-sm text-gray-500">
                    Are you sure you want to delete this token? This action cannot be undone and will affect the user's API access.
                  </p>
                </div>
              </div>
            </div>
...
import type { MCPToken } from '../../models/mcpToken.model';
import { showSuccess, showError } from '../../utils/notification';

interface MCPTokenWithUser extends MCPToken {
  userId?: number;
}

const loading = ref(false);
const tokens = ref<MCPTokenWithUser[]>([]);
const total = ref(0);
...
async function loadTokens() {
  ...
  try {
    ...
    const response = await mcpTokenService.listAllTokens(query);
...
async function deleteToken() {
  ...
  loading.value = true;
  try {
    const response = await mcpTokenService.adminDeleteToken(deleteConfirmation.value.tokenId);
...
async function deactivateToken(tokenId: number) {
  loading.value = true;
  try {
    const response = await mcpTokenService.adminDeactivateToken(tokenId);
...
async function activateToken(tokenId: number) {
  loading.value = true;
  try {
    const response = await mcpTokenService.adminActivateToken(tokenId);
